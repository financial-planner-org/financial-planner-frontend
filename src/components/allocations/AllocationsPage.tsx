...
export function AllocationsPage({
  ...
  if (isLoading) {
    return (
      <PageLayout>
        ...
        <LoadingState message='Loading allocations...' />
      </PageLayout>
    );
  }
  ...
  return (
    <PageLayout>
      <PageHeader
        title='Allocations'
        description='Manage your financial and immobilized allocations'
        actions={
      ...
      {allocations.length === 0 ? (
        <EmptyState
          ...
          action={<Button onClick={onAddAllocation}>Create first allocation</Button>}
        />
      ) : (
        <div className='space-y-6'>
          ...
          <div className='grid grid-cols-1 md:grid-cols-3 gap-4'>
            ...
            <DataCard
              ...
              description='Allocations with financing'
            />
          </div>

          {/* Allocations list */}
          <PageCard title='Allocations List'>
        ...
        <div className='flex items-start justify-between'>
          <div className='flex-1'>
            ...
            <div className='flex items-center gap-2 mb-2'>
              <h3 className='text-lg font-semibold text-foreground truncate'>{allocation.title}</h3>
              <div className='flex gap-2'>
                <StatusBadge
                  status={getTypeLabel(allocation.type)}
                  type={allocation.type === 'FINANCIAL' ? 'financial' : 'immobilized'}
                />
                {allocation.isFinanced && <StatusBadge status='Financiado' type='financed' />}
              </div>
            </div>

            {/* Informações da alocação */}
            <div className='space-y-1 text-sm text-muted-foreground'>
              {allocation.startDate && <p>Início: {allocation.startDate}</p>}
              {allocation.period && <p>{allocation.period}</p>}
              {allocation.progress && (
                <div>
            ...
            {/* Última atualização */}
            <div className='flex items-center gap-2 mt-2'>
              {allocation.hasWarning && <AlertTriangle className='w-4 h-4 text-yellow-500' />}
          ...
          <div className='flex items-center gap-2 ml-4'>
            <Button variant='outline' size='sm' onClick={() => onUpdate(allocation.id)}>
              <Edit className='w-4 h-4 mr-1' />
              {UI_LABELS.BUTTONS.UPDATE}
            </Button>
            <Button variant='ghost' size='sm' onClick={() => onView(allocation.id)}>
              {UI_LABELS.BUTTONS.VIEW}
            </Button>
