...
import { Allocation, AllocationRecord, CreateAllocationInput, UpdateAllocationInput, CreateAllocationRecordInput } from '@/lib/types/core';

// Endpoints para alocações
export const allocationsApi = {
    // Listar alocações por simulação
    getAllocations: async (simulationId: string): Promise<Allocation[]> => {
        const response = await apiClient.get<ApiResponse<Allocation[]>>(`/simulations/${simulationId}/allocations`);
    ...
    getAllocationHistory: async (allocationId: string): Promise<AllocationRecord[]> => {
        const response = await apiClient.get<ApiResponse<AllocationRecord[]>>(`/allocations/${allocationId}/history`);
        return response.data.data;
    },

    // Criar registro de alocação
    createAllocationRecord: async (data: CreateAllocationRecordInput): Promise<AllocationRecord> => {
        const response = await apiClient.post<ApiResponse<AllocationRecord>>('/allocations/records', data);
        return response.data.data;
    },

    // Atualizar registro de alocação
    updateAllocationRecord: async (id: string, data: Partial<CreateAllocationRecordInput>): Promise<AllocationRecord> => {
        const response = await apiClient.put<ApiResponse<AllocationRecord>>(`/allocations/records/${id}`, data);
        return response.data.data;
    },
    ...
    updateAllocationValue: async (id: string, value: number): Promise<AllocationRecord> => {
        const response = await apiClient.post<ApiResponse<AllocationRecord>>(`/allocations/${id}/update`, { value });
        return response.data.data;
    }
};
