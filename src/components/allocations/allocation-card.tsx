    ...
    <div className={ALLOCATIONS_CONFIG.allocationCard.container}>
      ...
      <div className='space-y-3'>
        ...
        <div className='flex items-center gap-2'>
          ...
          {totalValue && (
            <span className={ALLOCATIONS_CONFIG.allocationCard.totalValue}>{totalValue}</span>
          )}
        </div>
        ...
        <div className='flex justify-end'>
          ...
          >
            ...
            <span className={ALLOCATIONS_CONFIG.allocationCard.updateButtonText}>Atualizar</span>
          </Button>
        </div>

        {/* Date information */}
        {date && <div className={ALLOCATIONS_CONFIG.allocationCard.date}>Início: {date}</div>}

        {dateRange && (
          <div className={ALLOCATIONS_CONFIG.allocationCard.dateRange}>{dateRange}</div>
        )}
        ...
        <div className='flex items-center gap-2 flex-wrap'>
          {badges.map((badge, index) => (
            <div key={index} className={ALLOCATIONS_CONFIG.badges[badge.type]}>
              <span className={ALLOCATIONS_CONFIG.badges[`${badge.type}Text`]}>{badge.label}</span>
            </div>
          ))}
        </div>
        ...
        {progress && (
          <div className='space-y-2'>
            <div className={ALLOCATIONS_CONFIG.allocationCard.progress}>{progress}</div>
        ...
        {/* Update type */}
        {updateType && (
          <div className={ALLOCATIONS_CONFIG.allocationCard.updateType}>{updateType}</div>
        )}

        {/* Last update with warning */}
        <div className='flex items-center gap-2'>
          {hasWarning && <AlertTriangle className='h-4 w-4 text-orange-500' />}
          <span
            className={
              hasWarning
                ? ALLOCATIONS_CONFIG.allocationCard.lastUpdateWarning
                : ALLOCATIONS_CONFIG.allocationCard.lastUpdate
            }
          >
