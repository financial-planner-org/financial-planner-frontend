...
import { Simulation, Projection, CreateSimulationInput, UpdateSimulationInput } from '@/lib/types/core';
...
export const simulationsApi = {
    ...
    duplicateSimulation: async (id: string, name: string): Promise<Simulation> => {
        const response = await apiClient.post<ApiResponse<Simulation>>(`/simulations/${id}/duplicate`, { name });
        return response.data.data;
    },
    ...
    // Obter projeção
    getProjection: async (id: string, status: 'Vivo' | 'Morto' | 'Inválido'): Promise<Projection[]> => {
        const response = await apiClient.get<ApiResponse<Projection[]>>(`/simulations/${id}/projection`, {
            params: { status }
        });
        return response.data.data;
    },
