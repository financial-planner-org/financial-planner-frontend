...
export function HistoryPage({
  ...
  return (
    <PageLayout>
      <PageHeader title='Simulation History' description='View and manage all saved simulations' />
      ...
      {history.length === 0 ? (
        ...
        <div className='space-y-6'>
          ...
          <div className='grid grid-cols-1 md:grid-cols-4 gap-4'>
            ...
            />
            <DataCard
              title='Average Patrimony'
              value={formatCurrency(
                history.reduce((sum, item) => sum + item.totalPatrimony, 0) / history.length
              )}
              description='Average simulation value'
            />
          </div>

          {/* Simulations list */}
          <PageCard title='Saved Simulations'>
    ...
    <div className='flex items-center gap-4 p-6 border rounded-lg bg-card'>
      ...
      <div className='w-12 h-12 bg-gradient-to-r from-orange-400 to-red-600 rounded-full flex items-center justify-center flex-shrink-0'>
        <div className='w-8 h-8 bg-background rounded-full flex items-center justify-center'>
          <span className='text-orange-500 font-bold text-sm'>{item.version}</span>
        </div>
      </div>
      ...
      <div className='flex-1 min-w-0'>
        <div className='flex items-center gap-2 mb-1'>
          <h3 className='text-xl font-semibold text-foreground truncate'>{item.simulationName}</h3>
          {item.isLegacy && <LegacyWarning tooltip='Versão legada – não editável' />}
        </div>

        <div className='text-sm text-muted-foreground mb-2'>
          Client: {item.clientName} • Version {item.version} • {item.createdAt}
        </div>

        <div className='flex items-center gap-6 text-sm'>
          <span className='text-foreground'>
            Patrimony: <strong>{formatCurrency(item.totalPatrimony)}</strong>
          </span>
          <span className='text-foreground'>
            Retirement: <strong>{item.retirementYear}</strong>
          </span>
        </div>
      </div>
