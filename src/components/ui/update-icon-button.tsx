'use client';

import * as React from 'react';
import { cn } from '@/lib/utils';

interface UpdateIconButtonProps extends React.ButtonHTMLAttributes<HTMLButtonElement> {
  children?: React.ReactNode;
  isLoading?: boolean;
  loadingText?: string;
}

const UpdateIcon = ({ className }: { className?: string }) => (
  <svg
    width='18'
    height='18'
    viewBox='0 0 18 18'
    fill='none'
    xmlns='http://www.w3.org/2000/svg'
    className={className}
  >
    <path
      d='M17.9626 4.88142C17.9633 4.76956 17.9416 4.65867 17.8987 4.55512C17.8559 4.45157 17.7928 4.35738 17.7131 4.27796L14.0643 0.67423C13.9839 0.595457 13.8885 0.533135 13.7837 0.490838C13.6788 0.448542 13.5666 0.427102 13.4533 0.427749C13.34 0.427102 13.2278 0.448542 13.1229 0.490838C13.0181 0.533135 12.9227 0.595457 12.8423 0.67423L10.4069 3.07955L1.00104 12.3694C0.921283 12.4488 0.858183 12.543 0.815357 12.6465C0.772532 12.7501 0.750824 12.861 0.751479 12.9728V16.5765C0.751479 16.802 0.842145 17.0181 1.00353 17.1775C1.16492 17.3369 1.3838 17.4265 1.61204 17.4265H5.2608C5.38121 17.4329 5.50166 17.4143 5.61433 17.3719C5.727 17.3294 5.82938 17.264 5.91482 17.18L15.2691 7.89019L17.7131 5.52737C17.7916 5.44499 17.8556 5.35018 17.9024 5.24689C17.9107 5.17914 17.9107 5.11065 17.9024 5.0429C17.9064 5.00334 17.9064 4.96348 17.9024 4.92391L17.9626 4.88142ZM4.90797 15.7266H2.47259V13.3213L11.0179 4.88142L13.4533 7.28674L4.90797 15.7266ZM14.6667 6.08833L12.2313 3.68301L13.4533 2.4846L15.8801 4.88142L14.6667 6.08833Z'
      fill='#B1B1B1'
    />
  </svg>
);

/**
 * Botão de atualizar com estilo específico e ícone SVG
 *
 * @example
 * ```tsx
 * <UpdateIconButton onClick={handleUpdate}>
 *   Atualizar
 * </UpdateIconButton>
 *
 * <UpdateIconButton
 *   isLoading={isLoading}
 *   loadingText="Atualizando..."
 * >
 *   Atualizar
 * </UpdateIconButton>
 * ```
 */
const UpdateIconButton = React.forwardRef<HTMLButtonElement, UpdateIconButtonProps>(
  (
    {
      className,
      children = 'Atualizar',
      isLoading = false,
      loadingText = 'Atualizando...',
      disabled,
      ...props
    },
    ref
  ) => {
    const isDisabled = disabled || isLoading;

    return (
      <button
        className={cn(
          // Estilo base com paleta de cores
          'flex w-full sm:w-auto h-8 sm:h-9 md:h-10 px-3 sm:px-4 md:px-6 py-2 sm:py-3 justify-center items-center gap-2 flex-shrink-0',
          'rounded-[40px] border border-[#434343] bg-[#434343]',
          "text-[#B1B1B1] text-xs sm:text-sm font-medium font-['ABeeZee'] leading-loose",
          'hover:bg-[#454545] hover:border-[#9F9F9F] transition-all duration-200',
          'focus:outline-none focus:ring-2 focus:ring-[#9F9F9F] focus:ring-offset-2 focus:ring-offset-[#101010]',
          'disabled:opacity-50 disabled:cursor-not-allowed',
          isLoading && 'cursor-not-allowed opacity-70',
          className
        )}
        ref={ref}
        disabled={isDisabled}
        {...props}
      >
        {isLoading ? (
          <>
            <div className='w-4 h-4 border-2 border-[#B1B1B1] border-t-transparent rounded-full animate-spin' />
            {loadingText}
          </>
        ) : (
          <>
            <UpdateIcon className='w-4 h-4' />
            {children}
          </>
        )}
      </button>
    );
  }
);
UpdateIconButton.displayName = 'UpdateIconButton';

export { UpdateIconButton, UpdateIcon };
